...
|--------|------------|------|-----------|
| **라이브러리 레이어** | `findata.sec`, `findata.dart` | SEC/DART API를 호출 → `list[dict]` 반환 | ❌ 없음 |
| **서버 레이어** | `findata.server` | FastAPI + SQLite 캐싱/영속 저장 | ✅ `~/.findata/` |

```
사용자 (ipynb, 스크립트)
    │
    ├── import findata            ← 라이브러리 레이어 (순수 함수, 디스크 접근 없음)
    │     ├── findata.sec.*       ← SEC EDGAR 데이터
    │     └── findata.dart.*      ← OpenDART 데이터
    │
    └── python -m findata.server  ← 서버 레이어 (FastAPI + DB)
          ├── api/                ← REST API 엔드포인트
          ├── db/                 ← SQLite 스키마 & 쿼리
          └── ingestion/          ← 배치 데이터 적재
```
...
│
├── findata/                       # ═══ 메인 패키지 ═══
│   ├── __init__.py                # 퍼블릭 API 표면 (4개 함수)
│   │
│   ├── core/                      # ── 공통 설정 ──
│   │   └── config.py              # DATA_DIR 경로 (~/.findata)
│   │
│   ├── sec/                       # ── SEC EDGAR 라이브러리 (순수 함수) ──
...
│   │   ├── company_data.py        # 서버 전용: 통합 오케스트레이터
...
│   │   └── app.py                 # ⚠️ 레거시 (server/app.py로 대체됨)
│   │
│   ├── dart/                      # ── OpenDART 라이브러리 ──
...
### 1. 라이브러리 ↔ 서버 분리
- **라이브러리 레이어**(`findata.sec`, `findata.dart`)는 순수 함수로, 외부 API를 호출하고 `list[dict]`를 반환합니다. 디스크에 아무것도 저장하지 않습니다.
...
pip install findata[server]      # + FastAPI, uvicorn, playwright
...
- 서버 레이어만 이 디렉토리에 접근 (DB 파일 생성/관리)
